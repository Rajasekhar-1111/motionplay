...
# Access vision module from tasks
vision = tasks.vision
...
class HandGestureRecognizer:
    ...
    Compatible with MediaPipe 0.10+ (uses HandLandmarker Tasks API).
    """
    
    def _ensure_model(self):
        """Download hand_landmarker.task model if not present (required for MediaPipe 0.10+)."""
    ...
    def __init__(self):
        """Initialize MediaPipe Hand Detector (uses new Tasks API from MediaPipe 0.10+)."""
        logger.info("="*70)
        ...
        # MediaPipe setup - create HandLandmarker with options
        base_options = tasks.BaseOptions(model_asset_path=model_file)
        ...
        logger.info(f"  - Presence confidence: 0.5")
    ...
    def detect_hand(self, frame):
        """
        Detect hand landmarks in frame using MediaPipe HandLandmarker.
        ...
        # Create MediaImage wrapper for the HandLandmarker API
        mp_image = vision_image.Image(image_format=vision_image.ImageFormat.SRGB, data=frame_rgb)
        
        # Process frame with HandLandmarker
